        ...
        private async Task HandleClient(TcpClient userTcp)
        {
            ...
            if (line is not null)
            {
                ...
                if (user is not null)
                {
                    ...
                    UserTcps.Where(x => x.User == user).ToList().ForEach(x =>x.Client.Close());
                    UserTcps.RemoveAll(x => x.User == user);
            ...
            while (client.Connected)
            {
                ...
                {
                    Console.WriteLine($"Ошибка при обработке сообщения от клиента: {ex.Message}");
                }
            }
